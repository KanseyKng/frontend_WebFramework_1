...
import { Badge } from '@/app/components/ui/badge'; // pastikan komponen Badge tersedia
...
export default function ApiTodoList({ initialTasks }: ApiTodoListProps) {
  ...
  const handleToggleTask = async (id: number, currentCompleted: boolean) => {
    ...
    try {
      await todosService.updateTodoStatus(id, targetStatus);
    } catch (err) {
  ...
  const handleDeleteTask = async (id: number) => {
    ...
    try {
      ...
      console.warn('Simulasi delete ke API DummyJSON gagal (fallback state):', err);
      // Rollback jika perlu
    }
  };
